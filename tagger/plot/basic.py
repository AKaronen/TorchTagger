...
def training_history(save_path, history):
    """Plot training history from model training

    Args:
        save_path (str): Where to save the training history plot
    ...
    for metric, title, ylabel, subplot in [
        ...
        ax[subplot].set_xlabel("Epoch")
        ax[subplot].set_ylabel(ylabel)
        ax[subplot].grid(True)
        ax[subplot].legend(loc="best")
    plt.savefig(f"{save_path}/training_history.png", bbox_inches="tight")
    fig.clf()
...
def loss_history(plot_dir, loss_names, history):
    ...
    ax.plot(
        history.history["val_loss"], label="Validation Loss", linewidth=style.LINEWIDTH
    )
    ax.grid(True)
    # ax.set_ylabel('Loss')
    ax.set_ylabel("Loss")
...
def basic(model, signal_dirs):
    ...
    for i in range(-1, len(signal_dirs), 1):
        ...
        for class_pair in class_pairs:
            binary_dir = (
                os.path.join(sample_plot_dir, f"test_set") if i != -1 else plot_dir
            )
            ROC_binary(
                y_p, y_t, model.class_labels, binary_dir, class_pair, process_label
            )
            if i != -1:
        ...
        binary_dir_test = (
            os.path.join(sample_plot_dir, "test_set") if i != -1 else plot_dir
        )
