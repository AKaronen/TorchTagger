...
def train_weights(y_train, reco_pt_train, class_labels, weightingMethod, debug):
    ...
    if weightingMethod not in ["none", "ptref", "onlyclass"]:
        raise ValueError(
            "Oops!  Given weighting Method not defined in train_weights(). Use either none, ptref, or onlyclass."
        )
...
def train(model, data, out_dir, percent, use_numpy):

    # Load the data, class_labels and input variables name, not really using input variable names to be honest
    if use_numpy:
        X_path = os.path.join(data, "jetConstituent_150_4vec.npy")
        y_path = os.path.join(data, "jetConstituent_target_150.npy")
        data_train, data_test = load_np_data(
            X_path,
            y_path,
            percentage=percent,
        )
        X_train, y_train = data_train["inputs"], data_train["targets"]
        X_test, y_test = data_test["inputs"], data_test["targets"]
        save_test_data(out_dir, X_test, y_test, None, None)
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-y",
        "--yaml_config",
        default="tagger/model/configs/baseline_larger.yaml",
    ...
    parser.add_argument(
        "--np-data",
        action="store_true",
        help="Use numpy data files instead of root files",
    )
    args = parser.parse_args()
    with open(args.yaml_config, "r") as stream:
        yaml_dict = yaml.safe_load(stream)
    dataset = yaml_dict["data"]  # Path to data, either root files or numpy files
    ...
    if args.plot_basic:
        ...
        train(model, dataset, args.output, args.percent, args.np_data)
